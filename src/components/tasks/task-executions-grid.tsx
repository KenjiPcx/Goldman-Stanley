...
export function TaskExecutionsGrid({
	...
	const getRowCompletion = useCallback(
		(taskExecution: TaskExecution) => {
			if (!schema || schema.length === 0) {
				return { filled: 0, total: 0, percentage: 0 };
			}

			const item = enrichedResearch.find(
				(r) => r.taskExecution._id === taskExecution._id,
			);
		...
		if (schema && schema.length > 0) {
			schema.forEach((field) => {
				cols.push({
					...
					cell: ({ row }) => {
						const item = enrichedResearch.find(
							(r) => r.taskExecution._id === row.original._id,
						);
		...
		cols.push({
			...
			cell: ({ row }) => {
				...
				return (
					<Badge
						variant="outline"
						className={`${statusColors[status]} text-xs px-2 py-0`}
					>
						{status}
					</Badge>
				);
				...
				return (
					<div className="flex gap-1">
						...
						{task.status === "completed" && task.report && (
							<a href={`/task-execution/${task._id}`} target="_blank">
								<Button variant="ghost" size="sm" className="h-7 w-7 p-0">
				...
				<div className="h-full flex flex-col overflow-hidden">
					...
					<div className="border-b bg-muted/30 px-6 py-4 flex items-center justify-between flex-shrink-0">
						<div className="flex items-center gap-4">
							<div>
								<h2 className="text-xl font-semibold">Research Results Grid</h2>
								<p className="text-sm text-muted-foreground mt-1">
									{displayData.length} task executions • {schema?.length || 0}{" "}
									fields
								</p>
							</div>
						</div>
	...
	return (
		<div className="space-y-4 p-4">
			...
			{task.structuredOutput &&
				Object.keys(task.structuredOutput).length > 0 && (
					<>
						<div>
							<span className="font-medium block mb-3">Extracted Data:</span>
			...
			{task.report && (
				<div>
					<span className="font-medium block mb-2">Full Report:</span>
			...
			{task.error && (
				<>
					<Separator />
					<div>
						<span className="font-medium block mb-2 text-destructive">
							Error:
						</span>
				...
				{task.completedAt && (
					<div className="flex justify-between">
						...
						<span>
							...
							{Math.floor(((task.completedAt - task.startedAt) % 60000) / 1000)}
							s
						</span>
					</div>
				)}
